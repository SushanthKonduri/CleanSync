...
# https://github.com/elisemercury/CleanSync
...
class CleanSync:
    ...
    def __init__(self, input_data, mode='auto', duplicates=False, missing_num=False, missing_categ=False, encode_categ=False, extract_datetime=False, outliers=False, outlier_param=1.5, logfile=True, verbose=False):  
        ...
        mode (str)......................define in which mode you want to run CleanSync
                                        'auto' = sets all parameters to 'auto' and let CleanSync do the data cleaning automatically
        ...
        logfile (bool)..................define whether to create a logile during the CleanSync process
                                        logfile will be saved in working directory as "cleansync.log"
        verbose (bool)..................define whether CleanSync logs will be printed in console
        ...
        # initialize our class and start the cleansync process
        self.output = self._clean_data(output_data, input_data)  

        end = timer()
        logger.info('CleanSync process completed in {} seconds', round(end-start, 6))

        if not verbose:
            print('CleanSync process completed in', round(end-start, 6), 'seconds')
        if logfile:
            print('Logfile saved to:', os.path.join(os.getcwd(), 'cleansync.log'))
    ...
    def _initialize_logger(self, verbose, logfile):
        ...
        if logfile == True:    
            logger.add('cleansync.log', mode='w', format='{time:DD-MM-YYYY HH:mm:ss.SS} - {level} - {message}')
        return

    def _validate_params(self, df, verbose, logfile):
        # function for validating the input parameters of the cleansync process
    ...
    def _clean_data(self, df, input_data):
        # function for starting the cleansync process
